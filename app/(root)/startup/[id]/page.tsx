    ...
    <section className='section_container'>
        ...
        <div className='space-y-5 mt-10 max-w-4xl mx-auto'>
            <div className='flex-between gap-5'>
                <Link href={`/user/${post.author?.id}`} className='flex gap-2 items-center mb-2'>
                <Image
                ...
                />
                <div>
                    <p className='text-20-medium'>{post.author.name}
                    </p>
            ...
            <h3 className='text-30-bold'>Pitch Details</h3>
            {
                parsedContent ? (
                    <article
                    className='prose max-w-4xl font-work-sans break-all' dangerouslySetInnerHTML={{__html:parsedContent}}/>
                ):(
        ...
        {/* {editorPosts?.length > 0 &&(
            <div className='max-w-4xl mx-auto'>
                ...
                <ul className='mt-7 card_grid-sm'>
                    {editorPosts.map((post:StartTypeCard,index:number)=>(
